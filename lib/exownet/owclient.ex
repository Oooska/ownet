defmodule Exownet.OWClient do
  ...
  @maxsize 65536
  @type error_tuple :: {:error|:ownet_error, integer() | atom() | String.t()}

  @spec ping(:gen_tcp.socket(), OWPacket.flag_list()) :: {:ok, boolean()} | {:error, integer() | atom() | String.t(), boolean()}
  def ping(socket, flags \\ []) do
    flags = OWPacket.calculate_flag(flags, 0)
    req_packet = OWPacket.create_packet(:NOP, <<>>, flags)

    case send_and_receive_empty_response(socket, req_packet) do
      {:ok, _header, _payload, persistence} -> {:ok, persistence}
      error -> error
    end
  end

  @spec present(:gen_tcp.socket(), String.t, OWPacket.flag_list()) ::{:ok, binary(), boolean()} | {:error, atom() | String.t(), boolean()}
  ...
  @spec dir(:gen_tcp.socket(), String.t(), OWPacket.flag_list()) :: {:ok, list(String.t()), boolean()} |  {:error, integer() | atom() | String.t(), boolean()}
  ...
  defp send_and_receive_empty_response(socket, packet) do
    with :ok <- Socket.send(socket, packet),
         {:ok, header, payload, persistence} <- receive_message(socket) do
      {:ok, header, payload, persistence}
    else
      {@ownet_error, ret_code, persistence} -> {:error, ret_code, persistence}
  ...
  defp send_and_receive_response_with_payload(socket, packet) do
    with :ok <- Socket.send(socket, packet),
         {:ok, header, payload, persistence} <- receive_message_with_payload(socket) do
      {:ok, header, payload, persistence}
    else
      {@ownet_error, ret_code, persistence} -> {:error, ret_code, persistence}
      {:error, reason, false} -> {:error, reason, false}
      {:error, reason} -> {:error, reason, false}
    end
  end
  ...
  defp receive_packet(socket, num_bytes) do
    case Socket.recv(socket, num_bytes) do
      {:ok, packet} -> {:ok, packet}
 ...
 # defp close_socket(socket), do: :gen_tcp.close(socket)
  ...
  @spec receive_message(:gen_tcp.socket()) :: {:ok, OWPacket.header(), <<>>, boolean()} | {:ownet_error, integer(), boolean()} | error_tuple
  ...
  @spec receive_message_with_payload(:gen_tcp.socket()) :: {:ok, OWPacket.header(), <<>>, boolean()} | {:ownet_error, integer(), boolean()} | error_tuple
  defp receive_message_with_payload(socket) do
    case receive_message(socket) do
      {:ok, _header, <<>>, _persistence_granted} -> receive_message_with_payload(socket)
  ...
  @spec receive_header(:gen_tcp.socket()) :: {:ok, binary()} | error_tuple
  defp receive_header(socket) do
    receive_packet(socket, 24)
  end
  ...
  defp receive_payload(socket, header) do
    ...
    if payload_size > 0 do
      case receive_packet(socket, payload_size) do
